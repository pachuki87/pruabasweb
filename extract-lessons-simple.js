...
function extractCleanContent(html) {
    ...
    console.log('🔍 Buscando contenido educativo...');
    ...
    console.log('✅ Encontrado div ld-tab-content');
    ...
    contentDiv.find('p').each((i, element) => {
        ...
        if (text && 
            ...
            console.log(`📄 Párrafo encontrado: ${text.substring(0, 50)}...`);
        }
    });
    ...
    contentDiv.find('ul, ol').each((i, element) => {
        ...
        if (text && !text.includes('WordPress') && !text.includes('admin')) {
            ...
            console.log(`📋 Lista encontrada con ${$el.find('li').length} elementos`);
        }
    });
    
    console.log(`✨ Total elementos extraídos: ${cleanElements.length}`);
...
async function extractLessons() {
    ...
    if (!fs.existsSync(cursoPath)) {
        console.error(`❌ No existe la carpeta: ${cursoPath}`);
        return;
    }
