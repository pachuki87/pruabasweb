...
// netlify/functions/node_modules/proxy-from-env/index.js
var require_proxy_from_env = __commonJS({
  "netlify/functions/node_modules/proxy-from-env/index.js"(exports2) {
    "use strict";
    var parseUrl = require("url").parse;
    var DEFAULT_PORTS = {
      ftp: 21,
...
// netlify/functions/node_modules/follow-redirects/debug.js
var require_debug = __commonJS({
  "netlify/functions/node_modules/follow-redirects/debug.js"(exports2, module2) {
    var debug;
...
// netlify/functions/node_modules/follow-redirects/index.js
var require_follow_redirects = __commonJS({
  "netlify/functions/node_modules/follow-redirects/index.js"(exports2, module2) {
    var url = require("url");
    var URL2 = url.URL;
    var http = require("http");
    var https = require("https");
...
// netlify/functions/node_modules/axios/dist/node/axios.cjs
var require_axios = __commonJS({
  "netlify/functions/node_modules/axios/dist/node/axios.cjs"(exports2, module2) {
    "use strict";
...
var testWebhookConnectivity = async () => {
  ...
  try {
    console.log(`\u{1F50D} [${testId}] Probando conectividad del webhook...`);
    console.log(`\u{1F50D} [${testId}] URL: ${WEBHOOK_URL}`);
    const testPayload = {
    ...
    if (error.code === "ECONNREFUSED") {
      ...
      errorSolution = "El servidor n8n no est\xE1 respondiendo. Verifica que el servicio est\xE9 activo.";
    } else if (error.code === "ENOTFOUND") {
      errorType = "dns_resolution";
      errorSolution = "No se puede resolver el dominio. Verifica la URL del webhook.";
    } else if (error.code === "ETIMEDOUT") {
      errorType = "timeout";
      errorSolution = "El servidor tard\xF3 demasiado en responder. Aumenta el timeout o verifica el estado del servidor.";
...
var sendToWebhookWithRetry = async (payload, options = {}) => {
  ...
  while (attempt <= maxRetries) {
    ...
    try {
      ...
      const responseTime = Date.now() - startTime;
      console.log(`\u2705 [${attemptId}] Env\xEDo exitoso:`, {
        status: response.status,
        statusText: response.statusText,
        responseTime: `${responseTime}ms`,
        dataSize: JSON.stringify(response.data).length,
  ...
  console.error(`\u{1F480} [${requestId}] Todos los intentos fallaron despu\xE9s de ${maxRetries + 1} reintentos`);
...
var analyzeError = (error) => {
  ...
  if (error.code === "ECONNREFUSED") {
    return {
      ...
      solution: "El servidor n8n no est\xE1 respondiendo. Verifique que el servicio est\xE9 activo y accesible."
    };
  } else if (error.code === "ENOTFOUND") {
    return {
      type: "dns_resolution",
      solution: "No se puede resolver el dominio. Verifique la URL del webhook y la configuraci\xF3n DNS."
    };
  } else if (error.code === "ETIMEDOUT") {
    return {
      type: "timeout",
      solution: "El servidor tard\xF3 demasiado en responder. Aumente el timeout o verifique el estado del servidor."
    };
  } else if (error.code === "ECONNRESET") {
    return {
      type: "connection_reset",
      solution: "La conexi\xF3n fue reseteada. Intente nuevamente o verifique la estabilidad de la red."
    };
  } else if (error.response?.status === 404) {
    return {
      type: "not_found",
    ...
    return {
      type: "unauthorized",
      solution: "No autorizado. Verifique si el webhook requiere autenticaci\xF3n."
    };
  } else if (error.response?.status === 403) {
    return {
      type: "forbidden",
      solution: "Acceso denegado. Verifique la configuraci\xF3n de seguridad del webhook."
    };
  } else if (error.response?.status >= 500) {
    return {
      type: "server_error",
  ...
  return {
    type: "unknown",
    solution: "Error no categorizado. Verifique los logs del servidor para m\xE1s detalles."
  };
};
var validatePayload = (payload) => {
  const errors = [];
  if (!payload || typeof payload !== "object") {
    errors.push("El payload debe ser un objeto v\xE1lido");
...
exports.handler = async (event, context) => {
  console.log("\u{1F680} Iniciando funci\xF3n send-corrections...");
  console.log("M\xE9todo HTTP:", event.httpMethod);
  ...
  try {
    ...
    if (parsedBody.action === "test-webhook") {
      console.log("\u{1F9EA} Ejecutando prueba de conectividad...");
    ...
    if (data.type === "quiz-responses" && data.studentInfo && data.responses) {
      console.log("\u2705 Payload ya est\xE1 en formato correcto, us\xE1ndolo directamente");
      webhookPayload = data;
    } else {
      console.log("\u{1F504} Transformando payload al formato esperado...");
      webhookPayload = {
        type: "quiz-responses",
        timestamp: (/* @__PURE__ */ new Date()).toISOString(),
        ...
        studentInfo: {
          ...
          submittedAt: data.fechaEnvio || (/* @__PURE__ */ new Date()).toISOString()
        },
        quizInfo: {
          title: data.quizData?.titulo || data.titulo_cuestionario || "Evaluaci\xF3n",
    ...
    if (!validation.valid) {
      console.error("\u274C Payload inv\xE1lido:", validation.errors);
      return {
        statusCode: 400,
        body: JSON.stringify({
          success: false,
    ...
    console.log("\u{1F4CB} Enviando datos a n8n...");
    console.log("\u{1F4CB} URL del webhook:", WEBHOOK_URL);
    console.log("\u{1F4CB} Payload size:", JSON.stringify(webhookPayload).length, "bytes");
    const webhookResult = await sendToWebhookWithRetry(webhookPayload, {
      maxRetries: MAX_RETRIES,
      timeout: TIMEOUT
    });
    console.log("\u{1F4CA} Resultado del env\xEDo:", webhookResult);
    ...
    console.error("Error en la funci\xF3n:", error);
    return {
      statusCode: 500,
      body: JSON.stringify({
        success: false,
...
*/
//# sourceMappingURL=send-corrections.js.map
